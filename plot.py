...
person_source = bokeh.models.ColumnDataSource(
    dict(top=original_form, x=np.linspace(1, n_people, n_people))
)
...
legend = bokeh.models.Legend(
    items=[
        ...
        bokeh.models.LegendItem(label="the wealth proportion of the richest 5%",
                                renderers=[inequality_plot.renderers[1]]),
...
def update_original_form():
    if random_simulation:
        ...
        with open('data/data_csv/data.csv', 'r') as data:
            reader = csv.reader(data)
            next(reader)
            for tel_id, _, wealth in reader:
                original_form[int(tel_id) - 1] = int(wealth)//1000
...
if not random_simulation:
    button = bokeh.models.Button(label="Update")
    button.on_event(bokeh.events.ButtonClick, update)
    layout = bokeh.layouts.grid([wealth_plot, [inequality_plot, person_plot], button])#, sizing_mode='stretch_both')
else:
    wealth_plot.on_event(bokeh.events.Tap, on_tap)
    layout = bokeh.layouts.grid([wealth_plot, [inequality_plot, person_plot]])
